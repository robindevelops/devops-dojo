...
func AskAI(state *session.State) (string, error) {
	...
	if state.Mode == "interview" {
		prompt = fmt.Sprintf("You are a Senior DevOps Interviewer conducting a technical interview. The candidate is currently facing this incident: '%s'. Instead of giving them a hint, ask them a probing question to make them explain their thought process or how they would approach finding the root cause. Be professional and brief.", state.ActiveIncidentID)
	}
...
// GeneratePostMortem calls Gemini to review the completed session
func GeneratePostMortem(sessionData map[string]interface{}) (string, error) {
	apiKey := os.Getenv("DOJO_API_KEY")
	if apiKey == "" {
		return "", fmt.Errorf("no DOJO_API_KEY found")
	}

	sessionBytes, _ := json.MarshalIndent(sessionData, "", "  ")
	prompt := fmt.Sprintf("You are a Senior DevOps Mentor reviewing a junior engineer's recently completed incident response session. Here is the session data:\n\n%s\n\nPlease provide a brief post-mortem review. Praise them for what they did well, point out if they relied too heavily on hints, and suggest one specific topic they should study next to improve.", string(sessionBytes))
